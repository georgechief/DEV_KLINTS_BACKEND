...
**Owner track:** Maheep (`docs/maheep/`) — **BE primary · FE only if irreversible disclosure / Fix copy needs wire**  
...
Read:
- docs/maheep/PRD_WB_08_CATALOGUE_WAVE2_SP01_LE01.md (this file)
- docs/maheep/WRITEBACK_SURFACE_MATRIX.md
- dataruns/writebacks/WRITEBACK_POSSIBLE_NOT_SHEET.csv
- dataruns/writebacks/mappings/registry.json
...
Ship:
1. Keep SP-01 disabled stub (not in MVP1 42) — do not deep-ship (§2 lock).
2. Enable LE-01 registry with irreversible disclosure + honest rollback limits (§5).
3. Seed WritebackAllowedCheck for **LE-01**; remove SP-01 via `0036` (§6).
4. Update possible sheet + SURFACE matrix rows (§7).
5. Transform/evidence paths work for FAIL/WARN worklist rows incl. nested `value` (§5.3).
6. Keep all WB-03…07 gates: Settings OFF default · once-per-run · claim-before-write · PII mask · audit (§8).
7. Tests + verify script (§10). No video evidence required.
...
|-------|----------------|-----|
| **SP-01** Tag consolidation | **No** | **Not in MVP1 42** (`CHECK_MASTER_42` has SP-08/SP-12 only); no DCS executor — keep disabled stub |
...
| Case | Expect |
|------|--------|
| Registry loads **LE-01** enabled; **SP-01** stays disabled stub | SP-01 `operations=[]` |
| Preview LE-01 with fixture / nested worklist evidence | ≥1 `event_ingest` intent with externalId |
| Execute denied when Settings OFF | `writebacks_disabled` |
| Execute denied when not allowlisted | SP-01 / CI-03 / LE-04 |
| Allowlist contains **LE-01** after migrate (`0036` removes SP-01) | |
| LE-01 mapping `irreversible` true | exposed to FE/API in preview payload |
| Concurrent double Approve | 409 (WB-07) |
| Rollback LE-01 | not supported / does not claim full undo |
...
Checks: registry LE-01 enabled · SP-01 disabled stub · allowlist LE-01 only · possible sheet · LE-04 / CI-03 still disabled.

Optional FE: only if disclosure copy is asserted via string fixture — otherwise BE verify is enough.

---

## 11. Staging proof (optional)

Acceptance is **code + tests + `verify_wb08_catalogue_wave2.py`**. No video recording required for WB-08.

Optional manual smoke (not a PR deliverable): Settings ON → LE-01 Fix disclosure → Approve; Settings OFF → denied. SP-01 stays out of scope.

---

## 12. Acceptance checklist

**Product lock:** SP-01 is **not** in MVP1 42 (`CHECK_MASTER_42` has SP-08/SP-12 only). Do not ship SP-01 writeback. WB-08 execute surface = **LE-01 only**.

- [x] SP-01 stays **disabled stub** (`enabled=false`, `operations=[]`); not allowlisted; not on FE Approve list  
- [x] LE-01 registry `enabled=true`; irreversible disclosure visible before Approve  
- [x] `WritebackAllowedCheck` seeds **LE-01** only for WB-08 (`0036` removes SP-01); CI-03 / LE-04 still off  
- [x] Possible sheet + SURFACE matrix: SP-01 `disabled` / LE-01 `yes`  
- [x] Settings OFF → LE-01 does not execute  
- [x] Settings ON → LE-01 Approve writes via existing Manago adapter  
- [x] ~~SP-01 rollback~~ — N/A (not shipped)  
- [x] LE-01 does not claim full reverse  
- [x] WB-04/07 once-per-run + claim-before-write still hold  
- [x] Audit events include `check_id` / `job_id` / `data_run_id` / **`irreversible`** on `writeback.executed`  
- [x] No edits under `dataruns/use_cases/handoff*` / FE `/handoff`  
- [x] Verify script green  
- [x] No video evidence required for WB-08
...
| Model / file | Change |
|--------------|--------|
| `WritebackAllowedCheck` | + LE-01 via migration; SP-01 removed by `0036` |
| `registry.json` | enable LE-01 only; SP-01 stays `enabled: false` |
| `SP-01.tag_consolidation.v1.json` | stub: `enabled: false`, `operations: []` |
| `LE-01.event_backfill.v1.json` | `enabled: true` (ops already present) |
| Possible CSV | LE-01 yes / limited; SP-01 disabled |
...
Unchanged envelope; LE-01 appears as:
...
```json
{
  ...
  "diff_hash": "…",
  "irreversible": true,
  "operator_disclosure": "Bulk PURCHASE event backfill may not be fully reversible. Rollback best-effort only.",
  "intents": [
    {
      "op_kind": "event_ingest",
      "target": "manago",
      "entity_key": "<masked>",
      "payload": { "externalId": "<order.id>", "email": "…", "contactExtEventType": "PURCHASE" }
    }
  ]
}
```
...
| Path | Role |
|------|------|
| `dataruns/writebacks/mappings/SP-01.tag_consolidation.v1.json` | Keep stub (`operations=[]`) |
| `dataruns/writebacks/mappings/LE-01.event_backfill.v1.json` | Enabled irreversible event_ingest |
| `dataruns/writebacks/mappings/registry.json` | LE-01 on; SP-01 off |
| `dataruns/writebacks/adapters/manago.py` | `event_ingest` execute |
| `dataruns/writebacks/transform.py` | LE-01 enrich + flatten worklist nesting |
| `dataruns/writebacks/gates.py` | allowlist |
| `dataruns/writebacks/pipeline.py` / `run_gate.py` | execute order + irreversible audit |
...
| `docs/maheep/WRITEBACK_SURFACE_MATRIX.md` | docs SoT |
...
- BE title: `feat(WB-08): enable LE-01 purchase event writeback`  
- FE title (only if disclosure needed): `feat(WB-08): LE-01 irreversible disclosure on Fix`  
- PR body: link this PRD · “independent of HO-02”  
