...
Read:
- docs/maheep/PRD_WB_02B_POSSIBLE_SHEET_RUNTIME_AND_DEMO.md
- docs/maheep/PRD_WB_02_FIX_APPROVE_WRITEBACK_EXECUTE.md
- docs/maheep/PRD_FE_12_FIX_EVIDENCE_EXCEL_DOWNLOAD.md
...
3. docs/maheep copy may remain as human mirror only; update PRD-WB-01C path note
...
| Body | `"Possible/not sheet not found: /app/docs/maheep/WRITEBACK_POSSIBLE_NOT_SHEET.csv"` |
| Cause | `possible_sheet.py` resolves `docs/maheep/…csv`; **`.dockerignore` excludes `docs/`** so the file never enters the image |
...
| `docs/maheep/WRITEBACK_POSSIBLE_NOT_SHEET.csv` | Optional human mirror / PRD companion — **not required in `/app`** |
...
| Item | Link |
|------|------|
| Incident | Prod 500 on `/writebacks/possible/` · path `/app/docs/maheep/WRITEBACK_POSSIBLE_NOT_SHEET.csv` |
