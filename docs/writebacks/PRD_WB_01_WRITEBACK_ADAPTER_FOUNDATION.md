...
## 1. Why this (and why Engineering)
...
**Engineering fit:** He owns connector surface (Shopify refresh, Manago v3 key, Integrations). Writers are the natural next layer.
...
Engineering **must** be able to prove writebacks work on connected **test** Manago + Shopify accounts — not only dry-run mocks.
...
Default: **BE only** for Engineering this sprint.
