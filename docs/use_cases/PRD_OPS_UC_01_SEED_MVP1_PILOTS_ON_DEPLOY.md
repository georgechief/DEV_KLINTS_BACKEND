...
**Owner track:** Engineering  — **BE / deploy** (tiny FE only if empty-state copy)  
...
**Out of scope:** New pilots · CAP-01 Matrix · Handoff · Engineering writebacks · changing blueprint JSON  
...
**Implemented: Option A** — `Dockerfile` `web` `CMD` runs `load_use_case_pilots` immediately after `migrate` (idempotent upsert). Post-deploy verify: `scripts/verify_use_case_pilots.py` (also invoked from `deploy/scripts/deploy.sh`).

| Option | Approach |
|--------|----------|
| **A (preferred)** ✅ | Deploy / entrypoint after migrate: `python manage.py load_use_case_pilots` (idempotent upsert) |
