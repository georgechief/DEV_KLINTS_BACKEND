...
| Situation | Who |
|-----------|-----|
| FE-only: fix `friendlyEvidenceElement` resolve order / placeholder `"—"` | Engineering alone |
| Additive API fields on evidence rows (`element`, `api_key`, `db_key`, `entity`) via worklist normalize | Engineering; **ping Engineering** for a quick review on the PR |
